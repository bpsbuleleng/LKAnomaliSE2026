...
7. **`Variabel Hitungan`** (BARU, dibuat aplikasi sendiri 2026-07-09 — BUKAN diimpor user, jangan bingung dengan tab 1-6 yang isinya data BPS). Kolom: `field_id`, `jenis`, `formula`. SPARSE by design: tab mulai KOSONG (cuma header) dan hanya berisi baris untuk computed field yang admin timpa formulanya lewat halaman config; field yang tak punya baris di sini pakai formula default di kode (`ComputedFields.js` `EDITABLE_DEFAULTS`). Reset ke default = HAPUS baris (bukan kosongkan sel). Lihat "Variabel hasil perhitungan (computed fields)" di bawah untuk field mana yang boleh diedit di sini.
...
### Variabel hasil perhitungan (computed fields) — read-only vs formula-editable

Computed field (`ComputedFields.js`) dibagi 2 kelompok, JANGAN dicampur:

1. **Formula-editable** — aritmetika FLAT (cuma `+ - * /` antar-field/angka, tanpa akses roster/tabel eksternal). Didefinisikan sebagai string di `EDITABLE_DEFAULTS` (bukan kode JS bebas), dievaluasi lewat parser AMAN yang sama dengan rule (`Formula.compileExpr`/`evaluateExpr` — TANPA eval, hanya tokenizer + recursive-descent). Admin boleh menimpa rumusnya lewat halaman config ("Variabel Hasil Perhitungan") — override tersimpan di tab `Variabel Hitungan` (skema no. 7). Daftarnya: `keluarga.b4r16`, `keluarga.luas_per_kapita`, `usaha.r26_total`, `usaha.pangsa_biaya_produksi`, `usaha.rasio_pendapatan_biaya`, `usaha.rasio_ntb`.
...
`DataAccess.getComputedFields(jenis)` (read-only, unprivileged) mengembalikan SEMUA field kedua kelompok — dipakai halaman admin supaya tidak "buta" terhadap variabel yang dipakai rule tapi tidak muncul di kuesioner. `DataAccess.updateComputedFieldFormula(adminPassword, jenis, fieldId, formula)` (privileged) menolak field yang bukan kelompok 1 (`NOT_EDITABLE`) dan formula yang gagal `Formula.compileExpr` (`INVALID_FORMULA`); `formula=''` = reset ke default. Override yang lolos validasi saat simpan TAPI entah kenapa gagal dievaluasi saat submit (mis. tab diedit manual di luar UI) **fallback diam-diam ke default** — formula admin TIDAK PERNAH boleh menggagalkan submit PML.
