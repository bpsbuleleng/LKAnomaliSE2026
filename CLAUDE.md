...
- **Strategi testing (paling beda dari rencana awal, WAJIB dipahami sebelum Fase 0)**:
  ...
  3. Deployment pertama (setting "Execute as" & "Who has access") kemungkinan besar bisa diatur lewat `webapp.access`/`webapp.executeAs` di `appsscript.json` saat `clasp deploy` — tapi ini belum pasti 100% konsisten di semua versi. **Fase 0 WAJIB verifikasi ini secara empiris** (coba, cek, laporkan). Kalau ternyata tidak terbawa otomatis, itu satu klik manual tambahan (sekali saja) yang harus dilakukan di editor online — laporkan supaya dokumen ini diupdate.
...
5. **U-series (usaha) — sebagian besar RESOLVED**: dimensi "usaha berdiri 2026 vs sebelumnya" disatukan (keputusan user: anggap sama, pakai field yang sama). Dimensi "usaha besar" MASIH belum dikonfirmasi apakah ikut disatukan juga atau tetap terpisah — lihat "Progress terjemahan" di bawah.
...
- Ke-16 anomali riil perlu diterjemahkan SATU-SATU dari `Identifikasi` (prosa) ke `when` (struktur) — ini butuh pemahaman domain form SE2026 (kode rincian kayak R13b1, R26b/R26f) yang gue TIDAK bisa nebak dengan aman. Rekomendasi: kerjakan ini di chat, sama seperti rencana semula — bukan karena venue-nya beda, tapi karena inilah pekerjaan yang perlu validasi paling ketat dari semua yang ada di proyek ini.
  **Yang masih gue butuh dari lo**: cuma satu — apakah dimensi "usaha besar" (beda dari dimensi tahun berdiri yang sudah disatukan) JUGA dianggap sama, atau tetap dianggap di luar cakupan v1? Detail per-anomali lainnya ada di "Progress terjemahan" di bawah.
...
```
U1: { "all": [{"field":"r13b1","op":"==","value":2}, {"field":"pangsa_biaya_produksi","op":">","value":0.5}] }   ← berlaku semua tahun berdiri; "usaha besar" masih nunggu keputusan
U2: { "field": "r27c", "op": "<", "field2": "r26_total" }   ← berlaku semua tahun berdiri; "usaha besar" masih nunggu keputusan
U3: { "all": [{"field":"r11a","op":"==","value":13}, {"any":[{"field":"r29c","op":">","value":0},{"field":"r29d","op":">","value":0}]}] }   ← berlaku semua tahun berdiri; "usaha besar" masih nunggu keputusan
...
8. Halaman **Kelola Pertanyaan & Rule** hanya bisa diakses lewat URL `?page=admin` + password admin — tidak muncul di navigasi PML sama sekali.
